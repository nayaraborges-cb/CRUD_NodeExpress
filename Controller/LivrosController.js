...
/*app.post("/livros", function(req, res){
    LivrosModel.create({
        ...
        ano_piblicacao: req.body.ano_publicacao,
        resumo: req.body.resumo
    }).then(function(){
        res.send(" ✅ Livro cadastrado com sucesso!");
    }).catch(function(erro){
        res.send(" ❌ Erro ao cadastrar o livro: " + erro);
    });
}); 

//READ
app.get("/", function(req, res){
    LivrosModel.findAll().then(function(livros){
        res.json({livros:livros});
    }).catch(function(erro){
        res.send(" 😕 Erro ao buscar os livros: " + erro);
    })
});

//Leitura para filtragem de um livro específico
app.get("/:titulo", function(req, res){
    LivrosModel.findAll({where: {"titulo": req.params.titulo}}).then(function(livro){
        res.send(livro);
        res.send(" Busca realizada com sucesso!");
    }).catch(function(erro){
        res.send(" 😞 Infelizmente não foi possível encontrar o livro: " + erro);
    });
});
...
app.put("/atualizar/:id", function(req, res){
    LivrosModel.update({
        ...
        ano_piblicacao: req.body.ano_publicacao,
        resumo: req.body.resumo},
        {where: {"id": req.params.titulo}}
    ).then(function(){
        res.send(" ✏️ Livro atualizado com sucesso!");
    }).catch(function(erro){
        res.send(" ❌ Erro durante a atualização do livro: " + erro);
    })
});

//DELETE
app.delete("/deletar/:id", function(req, res){
    LivrosModel.destroy({where: {"id": req.params.id}
    }).then(function(){
        res.send(" 🗑️ Livro deletado com sucesso!");
    }).catch(function(erro){
        res.send(" ❌ Erro ao deletar o livro: " + erro);
    })
});
